...
class ContrastiveEmbed(nn.Module):
    ...
    def forward(self, x, text_dict):
        ...
        # padding to max_text_len without using in-place indexed assignment
        cur_len = res.shape[-1]
        if cur_len < self.max_text_len:
            pad_len = self.max_text_len - cur_len
            pad_shape = (*res.shape[:-1], pad_len)
            pad = torch.full(pad_shape, float("-inf"), device=res.device)
            new_res = torch.cat([res, pad], dim=-1)
        else:
